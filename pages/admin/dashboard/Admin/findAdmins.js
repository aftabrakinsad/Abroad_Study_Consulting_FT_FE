...
export default function MyPage({ data}) {
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(`http://localhost:3001/admin/updateAdmin/${props.id}`, updatedData);
        ...
        <form onSubmit={handleFormSubmit}>   
          ...
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <svg aria-hidden="true" class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" stroke-linejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            </div>
...
export async function getServerSideProps({ query }) {
  ...
  try {
    const response = await axios.get('http://localhost:3001/admin/'+inputValue);
